...
import { Menu, LogOut, User, Sparkles, BookOpen, ChevronDown } from 'lucide-react';
...
const Navbar = ({ onToggleSidebar }) => {
  ...
  return (
    <nav className="sticky top-0 z-40 w-full bg-slate-950/80 backdrop-blur-md border-b border-slate-900/60 px-4 md:px-6 h-16 flex items-center justify-between">
        ...
        {user ? (
          <div className="relative">
            ...
            >
              ...
              <span className="hidden md:block text-xs font-semibold text-slate-300 pr-1 max-w-[120px] truncate">
                {user.name}
              </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/register"
              className="px-3 py-1.5 rounded-lg bg-brand-600 hover:bg-brand-500 text-xs font-bold text-white transition duration-300"
            >
              Sign Up
            </Link>
          </div>
