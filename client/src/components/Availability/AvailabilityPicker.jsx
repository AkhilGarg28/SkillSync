...
const AvailabilityPicker = ({ slots = [], timezone = 'UTC', onChange }) => {
  ...
  const handleAddSlot = () => {
    ...
    if (!validateTime(startTime) || !validateTime(endTime)) {
      setError('Please use HH:MM 24h format (e.g. 14:30)');
      return;
    }
    ...
    if (isDuplicate) {
      setError('This time window already exists');
      return;
    }
    ...
    <div className="space-y-6 select-none">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <select
          ...
          className="w-full md:max-w-xs bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder-slate-550 focus:outline-none focus:border-brand-500 transition duration-300"
        >
          {TIMEZONES.map((tz) => (
      ...
      <div className="space-y-2">
        <label className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
          ...
          Active Availability Windows ({slots.length})
        </label>

        {slots.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center p-8 border border-dashed border-slate-850 rounded-xl bg-slate-950/20">
            <Clock className="text-slate-650 mb-2" size={24} />
            <p className="text-xs text-slate-500">No slots defined yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {slots.map((slot, index) => (
              <div
                ...
                className="flex items-center justify-between p-3 rounded-xl border border-slate-800 bg-slate-900/40 text-slate-200"
              >
                <div>
